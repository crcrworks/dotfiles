...
フロントエンドの見た目を変更するコードを書いた場合（CSS の変更、レイアウト変更、コンポーネントの変更、色・フォント・サイズ・余白などのスタイル変更、アニメーションの追加、要素の追加・削除・移動など）、必要に応じて `agent-browser` を使ってブラウザで見た目を確認してください。

**デフォルトではこの確認手順はスキップします。ユーザーが `/browser` スラッシュコマンドを実行した場合のみ、以下の手順に従ってブラウザ確認を実施してください。**
...
3. **スクリーンショットの保存先を設定する**
   - スクリーンショットが Git に追跡されないよう、保存先を一時ディレクトリに設定します：
     ```bash
     export AGENT_BROWSER_SCREENSHOT_DIR="/var/folders/nw/1rhqbntd1118df625z9l82300000gn/T/opencode/screenshots"
     mkdir -p "$AGENT_BROWSER_SCREENSHOT_DIR"
     ```

4. **変更があったページ・コンポーネントを確認する**
   - 変更内容に応じて該当するページに移動し、インタラクションが必要な場合は行います。
   - スクリーンショットを撮ります：
     ```bash
     agent-browser screenshot --full <変更内容を表す名前>.png
     ```
     ※ ファイルは `$AGENT_BROWSER_SCREENSHOT_DIR` に保存されるため、Git に追跡されません。
   - スマホ/タブレットビューが必要な場合は、`--viewport` オプションを使用してサイズを変えて確認します。

5. **スクリーンショットを目視確認する**
   - 撮影したスクリーンショット（`$AGENT_BROWSER_SCREENSHOT_DIR/<変更内容を表す名前>.png`）を `vision` エージェントに subagent として渡し、崩れやレイアウトの異常がないか確認してもらいます。
...
`/browser` 実行時、以下のいずれかに該当する変更がある場合は確認を行ってください：
